...
// グローバル変数
//*****************************************************************************
static ID3D11Buffer				*g_VertexBuffer_fade = NULL;		// 頂点情報
static ID3D11ShaderResourceView	*g_Texture_fade[TEXTURE_MAX] = { NULL };	// テクスチャ情報

static char *g_TexturName_fade[] = {
...
HRESULT InitFade(void)
{
	...
	GetDevice()->CreateBuffer(&bd, NULL, &g_VertexBuffer_fade);
...
void DrawFade(void)
{
	...
	UINT offset = 0;
	GetDeviceContext()->IASetVertexBuffers(0, 1, &g_VertexBuffer_fade, &stride, &offset);
	...
	{
		// テクスチャ設定
		GetDeviceContext()->PSSetShaderResources(0, 1, &g_Texture_fade[g_TexNo]);

		// １枚のポリゴンの頂点とテクスチャ座標を設定
		//SetVertex(0.0f, 0.0f, SCREEN_WIDTH, TEXTURE_WIDTH, 0.0f, 0.0f, 1.0f, 1.0f);
		SetSpriteColor(g_VertexBuffer_fade, SCREEN_WIDTH/2, TEXTURE_WIDTH/2, SCREEN_WIDTH, TEXTURE_WIDTH, 0.0f, 0.0f, 1.0f, 1.0f,
			g_Color);
